...
## Indicators by Colonizing Country
...
ColMad <- read.csv("Data/ColMad.csv")
...
### Average GDP, Gini Coefficient, and Human Development Index by Colonized Country
```{r}
...
```{r}
ggplot(filter(England, year >= 1900 & year <= 1950), aes(x = year, y = cgdp)) + geom_point() + labs(title = "England/UK's GDP over time")
```
...
Let's see if we can uncover anything about England's GDP versus those of its colonies. Since the UK colonized so many countries we selected a few to look at based on which countries we had more data for.

```{r}
ggplot() + geom_line(data = filter(England, year >= 1800), aes(x = year, y = log(cgdp)), color = "red") + geom_line(data = filter(ColMad, ColRulerName == "United Kingdom" & year >= 1800), aes(x = year, y = log(cgdppc), col = country))

```
Even restricted to a narrower timeframe, there are too many countries to really interpret this graph. So we'll pull a few countries out based on their current GDP's

```{r}
ggplot() + geom_line(data = filter(England, year >= 1800 & year <= 1950), aes(x = year, y = cgdp), color = "red") + geom_line(data = filter(ColMad, (ColRulerName == "United Kingdom" & (GDPpc17 >= 30000) & year >= 1800 & year <= 1950)), aes(x = year, y = cgdppc, col = country))
```


## GDP by years colonized
```{r}
