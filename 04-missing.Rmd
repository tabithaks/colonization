...
```{r}
colonialdata <- read.csv("Data/colonialdata.csv")
...
```{r}
England <- read.csv("Data/EnglandGDPpc.csv")
...
Note: I think we can just take this out

Examine Missing Values for Maddison GDP

```{r fig.height = 20}
MadGDP <- read.csv("Data/MaddisonGDP.csv")

missvalues <- MadGDP %>%gather(key, value, -year) %>%mutate(missing =ifelse(is.na(value), "yes", "no"))
missvalues1800 <- filter(MadGDP, year > 1800) %>%gather(key, value, -year) %>%mutate(missing =ifelse(is.na(value), "yes", "no"))

m <- ggplot(missvalues1800, aes(x = key, y =year, fill = missing)) +geom_tile(color ="white") +ggtitle("Missing Values in Maddison GDP After 1800") +scale_fill_viridis_d() + theme_bw() + coord_flip() + labs(x = "Countries", y = "Years") + theme(axis.text.x = element_text(angle = 90)) + theme_minimal(base_size=12)

n <-  ggplot(missvalues, aes(x = key, y =year, fill = missing)) +geom_tile(color ="white") +ggtitle("Missing Values in Maddison GDP") +scale_fill_viridis_d() + theme_bw() + coord_flip() + labs(x = "Countries", y = "Years") + theme(axis.text.x = element_text(angle = 90)) + theme_minimal(base_size=12)

ggsave("MadGDPmissing1800.png", m, width=20, height=20)
ggsave("MadGDPmissing.png", n, width=20, height=20)
```
...
```{r fig.height = 20}
Mad <- read.csv("Data/Maddison.csv")
...
ggsave("MadPmissingGDPC.png", mc, width=20, height=20)
#ggsave("MadGDPmissingGDPR.png", mr, width=20, height=20)
ggsave("MadPmissingGDPC1800.png", mc1800, width=20, height=20)
...
## INSERT FILE HERE ONCE GITHUB UPLOADED
...
These are some countries that would likely be more interesting to look at in terms of gdp overtime, simply because the data is more available. In terms of countries that have been colonized, looking at the trends in Latin America may be particularly interesting.


```{r}
transformations = read.csv('Data/transformations.csv')
